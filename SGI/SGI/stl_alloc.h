...
public:
	static void* allocate(size_t n) //申请空间
	{
		...
		if (0 == result)//  表明申请失败
		{
			result = oom_malloc(n);// 补救函数  内存不足的申请空间函数 
		}
		return result;
	}
	static void  deallocate(void *p, size_t n) //对p所指的空间释放
	{
		free(p);
	}
	static void* reallocate(void *p, size_t old_sz, size_t new_sz) //realloc 重新开辟空间 将原来的大小变成新的大小空间
	{
		void *result = realloc(p, new_sz);
		if (0 == result)
		{
			result = oom_realloc(p, new_sz);//  补救函数
		}
		return result;
	}             //函数名          // 函数指针
...
private:
	static void* oom_malloc(size_t n)//  补救函数
	{
	...
	static void* oom_realloc(void *p, size_t new_sz)//补救
	{
		void(*my_malloc_handler)();
		void *result;
		for (;;)
...
template<int inst>
void(*__malloc_alloc_template<inst>::__malloc_alloc_oom_handler)() = 0;//  对上面void(*__malloc_alloc_oom_handler)();函数指针初始化
...
{
	if (n > __MAX_BYTES)//   如果大于的128个  直接调用一级空间配置器
