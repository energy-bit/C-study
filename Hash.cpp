...
public:
	void Insert(const Type &x)
	{
		...
		int index = Hash(x);
		//头插法
		HashNode<Type>*s = _Buynode(x);//  购买节点
		s->link = _ht[index];
		_ht[index] = s;
		_size++;
	}
	...
	{
		HashNode<Type> *p = Find(key);//  先查找  如果没有了  直接返回说明要删除的数据不存在
		if (p == nullptr)
			return;

		if (p->link != nullptr)//   如果p的下一个节点不为空的话  就让它后一个节点的data值覆盖给p  再让他指向它下一个的下一个节点 
		{
			p->data = p->link->data;//  这样的好处就是不用在重新找一篇  找他的前驱节点   
			p->link = p->link->link;
			p = p->link;
		}
		else
		{
			...
			if (prev == p)    //  这种情况就是  前驱刚好也是它  说明是第一个节点
				_ht[index] = nullptr;
			else             // 这种情况就是   删除的刚好是最后一个节点
			{
				while (prev->link != p)
					prev = prev->link;
